...
parser.add_argument('--embed', type=int, default=16, help='Number of embedded units for GCN.')
...
def train(train_loader, val_loader, adj):
    ...
    def adjust_learning_rate(optimizer, global_step, lr):
        """Sets the learning rate to the initial LR decayed by 10 every 30 epochs"""
    #    lr = args.lr * (0.1 ** (epoch // 30))
        lr = lr * pow( args.lr_decay , float(global_step// decay_steps) )
    ...
    for epoch in range(args.epochs):
        ...
        cur_lr = adjust_learning_rate(optimizer, global_step, args.lr)
        ...
        if (epoch+1) % 10 == 0 and epoch != 0:
            ...
            print('epoch= {}, loss(train)= {:.3f}, acc(train)= {:.3f}, lr= {:.7f}'.format(epoch+1, epoch_loss, epoch_acc, cur_lr))
